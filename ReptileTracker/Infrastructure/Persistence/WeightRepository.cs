...
{
    public async Task<List<Weight>> GetAllForReptile(int reptileId, CancellationToken ct) => 
        await _context.Weights.Where(x => x.ReptileId == reptileId).ToListAsync(cancellationToken: ct);
}